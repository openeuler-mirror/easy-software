...
const pkgData = ref([]);
const PAGE_ID = 'PYPI';
const tabName = ref(PACKAGE_TYPE_MAPPING['pypi']);
...
const pageSearch = () => {
  ...
  if (tabName.value === PACKAGE_TYPE_MAPPING['pypi']) {
...
// 获取路由参数
const { routeKey, routeName, routeType, routeOs, routeArch, routeCategory } = useRouteQuery(PACKAGE_TYPE_MAPPING['pypi']);
...
<template>
  <div class="pkg-wrap" :class="tabName">
    ...
    <div class="pkg-main">
      <FilterHeader :title="PAGE_ID" :total="total" @search="changeFilterSearch" @clear="clearFilterSearch" />
