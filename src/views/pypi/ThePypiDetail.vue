  ...
  return [
    {
      ...
      installation: appData.value.pipUsage,
    },
  ];
});
...
const changePkgVersion = (v: string) => {
  ...
  windowOpen(`/zh/pypi/detail?pkgId=${current.pkgId}`, '_self');
};
</script>
...
<template>
  <ContentWrapper vertical-padding="24px">
    ...
    <template v-else>
      <!-- 锚点 -->
      <AppBreadcrumb id="pypi" :name="appData.name" />
      <!-- 头部信息 -->
      ...
      <div class="detail-row">
        ...
        <div class="detail-row-side">
          <DetailAside :data="appData" :ver-data="versionSupportData" :tagVer="tagVer" :maintainer="maintainer" type="PYPI" />
        </div>
      </div>
    </template>
  </ContentWrapper>
